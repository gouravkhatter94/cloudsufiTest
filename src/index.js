...
module.exports.handler = async (event, context) => {
    ...
    try {
        ...
        zipcode = zipcode && zipcode.toLowerCase().trim();
        ...
        objResponse.data = dbData.filter(objZipCode => {
            ...
            if (isPassed && zipcode) {
                isPassed = objZipCode.zip.indexOf(zipcode) > -1;
            }

            if (isPassed && cityName) {
                isPassed = objZipCode.primary_city.toLowerCase().indexOf(cityName) > -1;
            }
        ...
        if (latitude || longitude) {
            ...
            const arrDistantCities = objResponse.data.map(objZipCode => {
